...
namespace BuyProductsReport
{
    public class CadTable
    {
        ...
        public CadTableRow CreateRow(uint rowCount = 1)
        {
            var row = new CadTableRow(this);;
            for (int i = 0; i < rowCount; i++)
            {
                row = new CadTableRow(this);
                _rows.Add(row);
            }
            return row;
        }
        ...
        internal void MergeCells2(CadTableRow tableRow)
        {
            var current = (uint)tableRow.segments.Sum(t => t.length);
            ...
            for (int i = tableRow.segments.Count - 1; i >= 0; i--)
            {
                ...
                if (segment.length > 1)
                {
                    try
                    {
                        Table.MergeCells(tableRow.StartCell + current, (uint)(tableRow.StartCell + current +1));
                    }
                    catch
                    {
        ...
        public void InsertText(uint column, string text, CharFormat format)
        {
            uint col = 10 - column;
            Table.InsertText((uint)(Table.RowCount * Table.CellCount - col), 0, text, format);
        }

        public void InsertText(uint cell, string text)
        {
            Table.InsertText(cell, 0, text);
        }
        ...
        public void NewPage(int countSpaces = 0)
        {
            ...
            int countPastRow;
            if(countSpaces == 0)
                countPastRow = onPageLeft - 1 + countLinesOnOtherPages * countSpaces;
            else
                countPastRow= onPageLeft + countLinesOnOtherPages * countSpaces;

            if (countPastRow > 0)
                CreateRow((uint)countPastRow);
       }


    }
    public enum CadTableCellJust
    {
        Left = 0,
        Center = 1,
        Right = 2
